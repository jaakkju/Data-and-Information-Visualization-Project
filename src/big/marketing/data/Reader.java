...
//				tmpList.add(splitLine(sc.next().trim(), columns));
	...
	/** Reads in some csv tables from a zip file. The files inside the zipFile that are read in are specified by the regex
	 * streamName. This may be a single filename or a regex matching several files.
	...
	/**
	 * handles different splitting behaviour. The big brother data contains additional quotes, that have to be removed.
	 * All other data can be simply split at each ",".
...
//		test = r.read(IPS, 2); 			// 16600932 lines
	...
	public String [][] read(int type, int week){
		...
		if (week == 2){
			...
			switch (type) {
			case BIG_BROTHER:
				streamName="bb-week2.csv"; break;
			case FLOW:
				streamName="nf-week2.csv"; break;
			case IPS:
				streamName="IPS-syslog-week2.csv"; break;
			default:
			}
		}
		else{
